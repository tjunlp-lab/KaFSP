class ActionOperator:
    ...
    def approx(self, type_dict, value, interval=0.15):
        if value == 0:
            return set([k for k, v in type_dict.items() if v == 0])
        else:
            return set([k for k, v in type_dict.items() if v > value-value*interval and v < value+value*interval])

    def fuzzy_greater(self, type_dict, value, interval=0.15):
        return set([k for k, v in type_dict.items() if v > value-value*interval])

    def fuzzy_lesser(self, type_dict, value, interval=0.15):
        return set([k for k, v in type_dict.items() if v < value+value*interval])
